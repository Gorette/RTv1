...
void	put_pixel_to_image(t_dot d, t_data *data, char *img)
{
	...
	{
		img[((int)d.x * data->img->bpp) + ((int)d.y * data->img->s_l) + 0] = (char)255;
		img[((int)d.x * data->img->bpp) + ((int)d.y * data->img->s_l) + 1] = (char)255;
		img[((int)d.x * data->img->bpp) + ((int)d.y * data->img->s_l) + 2] = (char)255;
